...
const { createPopulation, SAVE_VERSION } = await import("../src/tank-state.js");
const { createTankStorage } = await import("../src/tank-storage.js");

const STEP = 1 / 60;
const saved = createPopulation({ age: 12 });
...
assert.equal(school.fish.length, COUNT, "a restored tank keeps its full population");
const liveIds = school.fish.map((f) => f.sid);
assert.equal(new Set(liveIds).size, COUNT, "restored ids are unique across the tank");
...
assert.equal(snapshot.fish.length, COUNT);

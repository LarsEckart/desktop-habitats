...
for (let frame = 0; frame < 7200; frame++) {
  ...
  for (const fish of school.fish) {
    ...
    assert.ok(
      Math.abs(fish.position.x) <= 6.45 &&
        fish.position.y >= 2.12 &&
        fish.position.y <= 6.22 &&
        fish.position.z >= 0.15 &&
        fish.position.z <= 3.05,
...
assert.deepEqual([...states].sort(), ["brake", "dart", "hover", "relocate"]);
assert.ok(
  mixedStates > 6500,
...
disturbed.dispose();
console.log(
  `PASS: 120 simulated seconds; all four behavior states; minimum sampled spacing ${minimumSpacing.toFixed(3)}; ${responses} local pointer responses followed by braking and recovery.`,
);
